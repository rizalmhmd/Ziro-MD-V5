...
let handler = async (m, { conn, args, usedPrefix, command }) => {
    ...
    let res = await fetch(`https://api.ibeng.tech/api/download/tiktok?url=${args[0]}&apikey=ibeng`).data;
...
handler.help = ['tiktokv3'].map(v => v + ' <url>')
handler.tags = ['downloader']
handler.command = /^(tik(tokv3)?(dl)?)$/i
